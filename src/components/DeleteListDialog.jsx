...
function DeleteListDialog({ dialogRef, open = false, id, name }) {
  ...
  const { deleteList: removeList } = listsSlice.actions;
  ...
  return (
    ...
    >
      <div className="text-white rounded-2xl bg-dark-06 w-[556px] p-16 relative">
        ...
        <h3 className="font-heading font-bold text-2xl mb-8">Deleting List</h3>
        <p className="font-body text-lg mb-12">
          Movies in this list won't get deleted. Are you sure you want to delete{" "}
          <b>{name}</b>?
        </p>
        <div className="flex gap-4">
          <Button className="bg-red! w-full text-white" onClick={deleteList}>
            Delete
          </Button>
